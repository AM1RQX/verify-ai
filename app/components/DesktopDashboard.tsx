...
export default function DesktopDashboard() {
    ...
    const [availableChecks, setAvailableChecks] = useState<number>(15);
        ...
        async function loadData() {
            ...
            if (usageData) {
                if (usageData.last_reset !== today) {
                    setAvailableChecks(15);
                } else {
                    setAvailableChecks(Math.max(0, 15 - usageData.scans_today));
                }
            }
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                    <div className="group relative overflow-hidden rounded-3xl border border-white/5 bg-zinc-900/40 p-8 backdrop-blur-sm transition-all hover:border-indigo-500/30">
                        ...
                        <div className="flex items-baseline gap-2 mt-4">
                            ...
                            <span className="text-zinc-600 font-medium">/ 15</span>
                        </div>
                        <div className="mt-6 h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                            <div
                                className="h-full bg-indigo-500 transition-all duration-1000"
                                style={{ width: `${(availableChecks / 15) * 100}%` }}
                            />
                        </div>
                    </div>
